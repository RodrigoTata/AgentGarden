...
Exhaustive static-analysis checklist. The auditor must account for **every** item — mark it as a finding or explicitly clear it with evidence.

Start with changed files (`git diff --name-only`), then widen to adjacent modules. Classify each finding: `🔴 CRITICAL` (exploitable now), `🟡 WARNING` (exploitable under conditions), `🔵 INFO` (defense-in-depth gap).

---

## Broken Access Control (Backend)

For **every** controller endpoint — not a sample, every one:
...
- `@Public()` only on login, register, health — any other = `🔴`
- Resource ownership verified — user A cannot access user B's data via UUID guessing
- Bulk endpoints validate ownership for **every** item in the batch
...
- `canActivate` guard present for admin-only routes — missing = `🟡`
...
- Parameter uses a typed DTO class — `@Body() body: any` = `🔴`
...
Run in all repos. Critical/High CVEs = `🔴`, Moderate = `🟡`, Low = `🔵`.
...
- Login response doesn't leak whether email exists
